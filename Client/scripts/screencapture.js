...
let ScreenCap = {
    ...
    return new Promise((resolve, reject) => {
      const result =
        range.rangeEnd > range.rangeStart
          ? dataUrls.concat([range.buffer])
          : dataUrls;

      console.log(dataUrls, 12312312);
      const arr = ScreenCap.concatUint8Array(
        ...result.map(
          (result) => new Uint8Array(ScreenCap.dataURItoArrayBuffer(result))
        )
      );
  ...
  readFileAsBlob: (range) => {
    return new Promise((resolve, reject) => {
      resolve(
        ScreenCap.readFileAsArrayBuffer(range).then(
  ...
  readFileAsDataURL: (range, withBase64Prefix = true) => {
    return ScreenCap.readFileAsBlob(range).then((blob) =>
      ScreenCap.blobToDataURL(blob, withBase64Prefix)
    );
  },
  ...
  init: function () {
    this.title =
      ...
      " - " +
      browser.i18n.getMessage("screenCapture");
    ...
    browser.runtime.onMessage.addListener(function (
      ...
      if (request.evt === "desktopcaptureData") {
        if (request.result.buffer) {
          resizedataURL(
            `data:application/octet-stream;base64,${request.result.buffer}`
          ).done(function (resultDataUri) {
            request.result.buffer = resultDataUri;
            $("#copyfish-tab-image").attr("src", `${request.result.buffer}`);
            browser.runtime.sendMessage({
              evt: "translateDesktopCapturedImage",
              data: `${request.result.buffer}`,
            });
...
function readTextareaContent() {
  ...
  fetch("http://localhost:3000/predict", {
    ...
    body: JSON.stringify({ input_data: textareaValue }),
  })
...
document
  .getElementById("readTextareaBtn")
  .addEventListener("click", readTextareaContent);
